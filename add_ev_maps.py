...
for d in os.listdir('ev'):
    ...
    # Try different patterns for where to insert
    if 'class="gas-link">Return Trip</a>' in html:
        html = re.sub(
            r'(<a href="/ev/[^"]+/" class="gas-link">Return Trip</a>)',
            r'\1\n            ' + maps_btn,
            html
        )
    elif '</div>\n        </div>\n        <div class="grid">' in html:
        # Add before grid section
        html = html.replace(
            '</div>\n        </div>\n        <div class="grid">',
            '</div>\n            ' + maps_btn + '\n        </div>\n        <div class="grid">'
        )
...
print(f"✅ Added Google Maps link to {count} UK EV pages")
